...
function ClientMapPageContent() {
  const clients = useClientMap(); const isSupervisor = getSessionIdentity()?.roles.includes("SUPERVISOR") ?? false; const [selectedId, setSelectedId] = useState<string | null>(null); const [focusVersion, setFocusVersion] = useState(0); const items = clients.result?.items ?? []; const hasFilters = Boolean(clients.search || clients.status || clients.withoutVisitSince || clients.withoutPurchaseSince); const initialError = clients.error !== null && clients.result === null; const refreshing = clients.loading && clients.result !== null; const mapPointer = useRef<{ x: number; y: number; dragged: boolean } | null>(null);
 ...
 return <section className="client-list client-map-page" aria-labelledby="client-map-page-title" onPointerDownCapture={(event) => { if ((event.target as Element).closest(".client-map__canvas")) mapPointer.current = { x: event.clientX, y: event.clientY, dragged: false }; }} onClickCapture={(event) => { const dragged = mapPointer.current?.dragged ?? false; mapPointer.current = null; if (!dragged && !(event.target as Element).closest("[data-client-map-selection]")) setSelectedId(null); }}><header className="client-list__heading"><div><h1 id="client-map-page-title">Mapa general de clientes</h1><p>Consulta la ubicación registrada de los clientes autorizados sin exponer datos de contacto.</p>{clients.lastUpdated && <p className="client-map-page__updated" role="status">Última actualización: <time dateTime={clients.lastUpdated.toISOString()}>{formatLastUpdated(clients.lastUpdated)}</time>.</p>}</div></header><section className="client-list__card" aria-label="Mapa de clientes"><ClientFilters query={clients.search} status={clients.status} territoryId={null} sellerId={null} withoutVisitSince={clients.withoutVisitSince} withoutPurchaseSince={clients.withoutPurchaseSince} options={{ territories: [], sellers: [] }} onQueryChange={clients.changeSearch} onStatusChange={clients.changeStatus} onTerritoryChange={() => undefined} onSellerChange={() => undefined} onWithoutVisitSinceChange={clients.changeWithoutVisitSince} onWithoutPurchaseSinceChange={clients.changeWithoutPurchaseSince} showAssignmentFilters={false} />{clients.error && !initialError && <InlineAlert variant="error" title="Ocurrió un problema temporal" message="No pudimos actualizar los clientes. El mapa mostrado puede no estar vigente." action={{ label: "Reintentar", onClick: clients.retry }} {...(clients.error.correlationId ? { correlationId: clients.error.correlationId } : {})} />}{clients.forbidden ? <AsyncStateCard tone="error" title="No tienes permisos" description="No tienes permiso para consultar el mapa de clientes." actionLabel="Reintentar" onAction={clients.retry} /> : initialError ? <AsyncStateCard tone="error" title="No pudimos cargar los clientes" description="No logramos obtener los clientes para el mapa. Reintenta en unos segundos." actionLabel="Reintentar" onAction={clients.retry} /> : clients.loading && items.length === 0 ? <TableLoadingIndicator label="Cargando clientes" /> : !clients.error && items.length === 0 && !hasFilters ? <AsyncStateCard title="Aún no hay clientes" description={emptyDescription} /> : <><div className="client-map-page__content"><ClientMap clients={items} selectedId={selectedId} focusVersion={focusVersion} onSelect={selectClient} empty={items.length === 0} /><ClientMapList clients={items} selectedId={selectedId} onSelect={selectClient} /></div>{refreshing && <div className="client-list__stale"><TableLoadingIndicator label="Actualizando clientes; se mantiene la última respuesta disponible y puede no estar vigente" compact /></div>}</>}</section></section>;
}
