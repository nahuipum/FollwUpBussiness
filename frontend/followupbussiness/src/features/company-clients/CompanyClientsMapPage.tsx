...
export function CompanyClientsMapPage() {
  ...
  useEffect(
    () => subscribeToSession(() => setSessionKey(clientSessionKey())),
    [],
  );
...
function ClientMapPageContent() {
  ...
  const isSupervisor =
    getSessionIdentity()?.roles.includes("SUPERVISOR") ?? false;
  ...
  const initialError = clients.error !== null && clients.result === null;
  const refreshing = clients.loading && clients.result !== null;
  const mapPointer = useRef<{ x: number; y: number; dragged: boolean } | null>(
    null,
  );
  useEffect(() => {
    const onMove = (event: PointerEvent) => {
      const start = mapPointer.current;
      if (
        start &&
  ...
  const selectClient = (clientId: string | null) => {
    ...
    if (clientId) setFocusVersion((value) => value + 1);
  };
  const emptyDescription = isSupervisor
    ? "No hay clientes disponibles para tu equipo en este momento."
    : "Cuando existan clientes aparecerán en este mapa.";
  return (
    <section
      className="client-list client-map-page"
      aria-labelledby="client-map-page-title"
      onPointerDownCapture={(event) => {
        if ((event.target as Element).closest(".client-map__canvas"))
          mapPointer.current = {
            x: event.clientX,
            y: event.clientY,
            dragged: false,
      ...
      onClickCapture={(event) => {
        const dragged = mapPointer.current?.dragged ?? false;
        mapPointer.current = null;
        if (
          !dragged &&
          !(event.target as Element).closest("[data-client-map-selection]")
        )
          setSelectedId(null);
      }}
    >
      <header className="client-list__heading">
        <div>
          ...
          {clients.lastUpdated && (
            <p className="client-map-page__updated" role="status">
              Última actualización:{" "}
              <time dateTime={clients.lastUpdated.toISOString()}>
                {formatLastUpdated(clients.lastUpdated)}
              </time>
              .
            </p>
          )}
      ...
      <section className="client-list__card" aria-label="Mapa de clientes">
        <ClientFilters
        ...
        {clients.error && !initialError && (
          <InlineAlert
            ...
            message="No pudimos actualizar los clientes. El mapa mostrado puede no estar vigente."
        ...
        {clients.forbidden ? (
          <AsyncStateCard
            ...
            description="No tienes permiso para consultar el mapa de clientes."
          ...
          <AsyncStateCard
            ...
            description="No logramos obtener los clientes para el mapa. Reintenta en unos segundos."
            actionLabel="Reintentar"
            onAction={clients.retry}
          />
        ) : clients.loading && items.length === 0 ? (
          <TableLoadingIndicator label="Cargando clientes" />
        ) : !clients.error && items.length === 0 && !hasFilters ? (
          <AsyncStateCard
          ...
          <>
            <div className="client-map-page__content">
              <ClientMap
                clients={items}
            ...
            {refreshing && (
              <div className="client-list__stale">
                <TableLoadingIndicator
                  label="Actualizando clientes; se mantiene la última respuesta disponible y puede no estar vigente"
                  compact
                />
              </div>
            )}
...
function ClientMapList({
  ...
  const end = Math.min(
    clients.length,
    Math.ceil((scrollTop + MAP_LIST_HEIGHT) / MAP_LIST_ROW_HEIGHT) +
      MAP_LIST_OVERSCAN,
  );
  ...
  return (
    <section
      ...
      aria-labelledby="client-map-list-title"
    >
      <header>
        <div>
          <h2 id="client-map-list-title">Clientes en el mapa</h2>
          <p>{clients.length} clientes. Selecciona uno para ubicarlo.</p>
        </div>
      </header>
      {clients.length === 0 ? (
        <p className="client-map-list__empty">No encontramos clientes.</p>
      ) : (
        <div
          className="client-map-list__viewport"
          role="list"
        ...
        >
          <div style={{ height: clients.length * MAP_LIST_ROW_HEIGHT }}>
            <ul
              style={{
                transform: `translateY(${start * MAP_LIST_ROW_HEIGHT}px)`,
              }}
            >
              {clients.slice(start, end).map((client) => (
                <li key={client.id} role="listitem">
                  <button
                    data-client-map-selection
                    type="button"
                    className={
                      selectedId === client.id
                        ? "client-map-list__item client-map-list__item--selected"
                        : "client-map-list__item"
                    }
                  ...
                  >
                    ...
                    <span
                      className={`client-map-list__status client-map-list__status--${client.status.toLowerCase()}`}
                    >
