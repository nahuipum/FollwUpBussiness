...
type Props = { territories: readonly Territory[]; page: number; pageSize: DataTablePageSize; totalPages: number; totalElements: number; lastUpdated?: Date | null; canManage: boolean; onPageChange: (page: number) => void; onPageSizeChange: (pageSize: DataTablePageSize) => void; onEdit: (territory: Territory) => void };

export function TerritoryTable({ territories, page, pageSize, totalPages, totalElements, lastUpdated, canManage, onPageChange, onPageSizeChange, onEdit }: Props) {
  ...
  return <><DataTable ariaLabel="Zonas" items={territories} rowKey={(territory) => territory.id} columns={columns} /><DataTablePagination page={page} totalPages={totalPages} pageSize={pageSize} onPageChange={onPageChange} onPageSizeChange={onPageSizeChange} ariaLabel="Paginación de zonas" summary={<>Mostrando {territories.length} de {totalElements} zonas</>} lastUpdated={lastUpdated} /></>;
}
