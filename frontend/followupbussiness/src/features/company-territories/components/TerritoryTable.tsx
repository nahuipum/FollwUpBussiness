...
type Props = { territories: readonly Territory[]; page: number; totalPages: number; totalElements: number; canManage: boolean; onPageChange: (page: number) => void; onEdit: (territory: Territory) => void };

export function TerritoryTable({ territories, page, totalPages, totalElements, canManage, onPageChange, onEdit }: Props) {
  ...
  return <><DataTable ariaLabel="Zonas" items={territories} rowKey={(territory) => territory.id} columns={columns} /><DataTablePagination page={page} totalPages={totalPages} onPageChange={onPageChange} ariaLabel="Paginación de zonas" summary={<>Mostrando {territories.length} de {totalElements} zonas</>} /></>;
}
