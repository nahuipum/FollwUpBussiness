...
export function useTerritories() {
  ...
  const [search, setSearch] = useState(""); const [status, setStatus] = useState<TerritoryStatus | null>(null); const [page, setPage] = useState(0);
  ...
  return { sessionKey: currentSessionKey, search, status, page, result, loading, error, lastUpdated, changeSearch: resetPage(setSearch), changeStatus: resetPage(setStatus), goToPage: setPage, retry: () => setReload((value) => value + 1), clearFilters: () => { setSearch(""); setStatus(null); setPage(0); } };
}
