...
const settings = { timezone: "America/Lima", currency: "PEN", geofenceRadiusMeters: 100, trackingIntervalSeconds: 60, locationRetentionDays: 90, saleEditWindowMinutes: 30 };
test("lee ETag contractual y lo reenvía literalmente en If-Match", async () => {
  ...
  await updateCompanySettings({ currency: "PEN", saleEditWindowMinutes: 30 }, read.snapshot!.etag);
  ...
  expect(JSON.parse(state.request.mock.calls.at(-1)?.[1].body)).toEqual({ currency: "PEN", saleEditWindowMinutes: 30 });
});
