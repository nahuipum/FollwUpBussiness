...
export function useCompanySettings() {
  const request = useRef(0); const generation = useRef(getSessionGeneration());
  const [snapshot, setSnapshot] = useState<CompanySettingsSnapshot | null>(null); const [sessionGeneration, setSessionGeneration] = useState(generation.current); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState<ApiError | null>(null); const [conflict, setConflict] = useState(false); const [lastUpdated, setLastUpdated] = useState<Date | null>(null);
