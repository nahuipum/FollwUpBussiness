...
import type { Seller, SellerFilters, SellerFormInput, SellerFormOptions, SellerPage, SellerReference, SellerStatus, TerritoryReference } from "./types";
...
type ReferencePage<T> = Readonly<{ items: readonly T[]; page: Readonly<{ page: number; pageSize: number; totalElements: number; totalPages: number }> }>;

function parseReferencePage<T>(value: unknown, parse: (entry: unknown) => T | null): ReferencePage<T> | null {
  if (typeof value !== "object" || value === null) return null;
  const result = value as Record<string, unknown>;
  if (!Array.isArray(result.items) || typeof result.page !== "object" || result.page === null) return null;
  const page = result.page as Record<string, unknown>;
  ...
  return items.some((item) => item === null) ? null : { items: items as T[], page: page as ReferencePage<T>["page"] };
}

function parseSupervisorOption(value: unknown): SellerReference | null {
  if (typeof value !== "object" || value === null) return null;
  const user = value as Record<string, unknown>;
  return typeof user.id === "string" && typeof user.displayName === "string" ? { id: user.id, displayName: user.displayName } : null;
}

function parseTerritoryOption(value: unknown): TerritoryReference | null {
  if (typeof value !== "object" || value === null) return null;
  const territory = value as Record<string, unknown>;
  return typeof territory.id === "string" && typeof territory.name === "string" && typeof territory.code === "string" ? { id: territory.id, name: territory.name, code: territory.code } : null;
}

async function listAllReferences<T>(path: string, parse: (entry: unknown) => T | null): Promise<{ response: Response; items: readonly T[] | null }> {
  const firstResponse = await apiRequest(`${path}&page=0&pageSize=100`, { method: "GET", headers: getSessionAuthorization() });
  const first = firstResponse.status === 200 ? parseReferencePage(await firstResponse.json().catch(() => null), parse) : null;
  if (!first || first.page.totalPages <= 1) return { response: firstResponse, items: first?.items ?? null };
  const rest = await Promise.all([...Array(first.page.totalPages - 1)].map(async (_, index) => {
    const response = await apiRequest(`${path}&page=${index + 1}&pageSize=100`, { method: "GET", headers: getSessionAuthorization() });
    return { response, page: response.status === 200 ? parseReferencePage(await response.json().catch(() => null), parse) : null };
  }));
  const failed = rest.find(({ response, page }) => response.status !== 200 || page === null);
  return failed ? { response: failed.response, items: null } : { response: firstResponse, items: [ ...first.items, ...rest.flatMap(({ page }) => page?.items ?? []) ] };
}

export async function listSellerFormOptions(): Promise<{ response: Response; options: SellerFormOptions | null }> {
  const [supervisors, territories] = await Promise.all([
    listAllReferences("/company/users?role=SUPERVISOR&status=ACTIVE", parseSupervisorOption),
    listAllReferences("/territories?status=ACTIVE", parseTerritoryOption),
  ]);
  if (supervisors.response.status !== 200) return { response: supervisors.response, options: null };
  if (territories.response.status !== 200) return { response: territories.response, options: null };
  return supervisors.items && territories.items ? { response: supervisors.response, options: { supervisors: supervisors.items, territories: territories.items } } : { response: supervisors.response, options: null };
}
...
function createBody(input: SellerFormInput) {
  return { displayName: input.displayName, email: input.email, ...(input.username ? { username: input.username } : {}), ...(input.phone ? { phone: input.phone } : {}), ...(input.employeeCode ? { employeeCode: input.employeeCode } : {}), ...(input.supervisorId ? { supervisorId: input.supervisorId } : {}), ...(input.territoryIds.length ? { territoryIds: [...new Set(input.territoryIds)] } : {}) };
}

export function createSeller(input: SellerFormInput) {
  return apiRequest("/sellers", { method: "POST", headers: mutationHeaders(), body: JSON.stringify(createBody(input)) });
}

export function updateSeller(seller: Seller, input: SellerFormInput) {
  return apiRequest(`/sellers/${encodeURIComponent(seller.id)}`, { method: "PATCH", headers: mutationHeaders(seller.version), body: JSON.stringify({ displayName: input.displayName, ...(input.phone ? { phone: input.phone } : {}), ...(input.employeeCode ? { employeeCode: input.employeeCode } : {}) }) });
}

export function updateSellerSupervisor(sellerId: string, supervisorId: string | null) {
  return apiRequest(`/sellers/${encodeURIComponent(sellerId)}/supervisor`, { method: "PUT", headers: mutationHeaders(), body: JSON.stringify({ supervisorId }) });
}

export function updateSellerTerritories(sellerId: string, territoryIds: readonly string[]) {
  return apiRequest(`/sellers/${encodeURIComponent(sellerId)}/territories`, { method: "PUT", headers: mutationHeaders(), body: JSON.stringify({ territoryIds: [...new Set(territoryIds)] }) });
}
