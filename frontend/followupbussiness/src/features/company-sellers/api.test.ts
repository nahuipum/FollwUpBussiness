...
import { createSeller, listSellerFormOptions, listSellers, updateSeller, updateSellerSupervisor, updateSellerTerritories } from "./api";
...
const seller = {
  ...
  phone: null, employeeCode: "VEN-001", status: "ACTIVE" as const, supervisorId: "supervisor-1",
...
test("carga referencias activas paginadas sin solicitudes por opción", async () => {
  state.request.mockResolvedValueOnce(new Response(JSON.stringify({ items: [{ id: "supervisor-1", displayName: "Sofía Supervisora" }], page: { page: 0, pageSize: 100, totalElements: 1, totalPages: 1 } }), { status: 200 }))
    .mockResolvedValueOnce(new Response(JSON.stringify({ items: [{ id: "territory-1", code: "LIM", name: "Lima Centro" }], page: { page: 0, pageSize: 100, totalElements: 1, totalPages: 1 } }), { status: 200 }));
  await expect(listSellerFormOptions()).resolves.toMatchObject({ options: { supervisors: [{ displayName: "Sofía Supervisora" }], territories: [{ code: "LIM" }] } });
  expect(state.request).toHaveBeenNthCalledWith(1, "/company/users?role=SUPERVISOR&status=ACTIVE&page=0&pageSize=100", expect.anything());
  expect(state.request).toHaveBeenNthCalledWith(2, "/territories?status=ACTIVE&page=0&pageSize=100", expect.anything());
});
...
test("envía creación y edición con precondición y asignaciones sin duplicados", async () => {
  ...
  const input = { displayName: "Ana Vendedora", email: "ana@example.com", username: "ana", phone: "999", employeeCode: "VEN-1", supervisorId: "supervisor-1", territoryIds: ["territory-1", "territory-1"] };
  ...
  expect(state.request).toHaveBeenNthCalledWith(1, "/sellers", expect.objectContaining({ method: "POST", body: JSON.stringify({ displayName: "Ana Vendedora", email: "ana@example.com", username: "ana", phone: "999", employeeCode: "VEN-1", supervisorId: "supervisor-1", territoryIds: ["territory-1"] }) }));
  expect(state.request).toHaveBeenNthCalledWith(2, "/sellers/seller-1", expect.objectContaining({ method: "PATCH", headers: expect.objectContaining({ "If-Match": "1" }) }));
  expect(state.request).toHaveBeenNthCalledWith(3, "/sellers/seller-1/supervisor", expect.objectContaining({ method: "PUT", body: JSON.stringify({ supervisorId: null }) }));
  expect(state.request).toHaveBeenNthCalledWith(4, "/sellers/seller-1/territories", expect.objectContaining({ method: "PUT", body: JSON.stringify({ territoryIds: ["territory-1"] }) }));
});
