...
export function useSessionRoute() {
  ...
  const [showInvalidSession, setShowInvalidSession] = useState(() => window.location.pathname !== '/' && !hasSession())
  ...
  const closeInvalidSession = useCallback(() => setShowInvalidSession(false), [])
