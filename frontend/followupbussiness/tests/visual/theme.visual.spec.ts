  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path.endsWith("/platform/companies")) {
      await route.fulfill({
        ...
        body: JSON.stringify({
          items: [],
          page: { page: 0, pageSize: 20, totalElements: 0, totalPages: 0 },
        }),
      });
      return;
    }
...
for (const viewport of [
  ...
  test(`modo oscuro ${viewport.name}: menú de perfil y persistencia visual`, async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Aún no hay empresas creadas" }),
    ).toBeVisible();
    ...
    expect(
      await page.evaluate(() =>
        window.localStorage.getItem("followupbusiness.theme"),
      ),
    ).toBe("dark");
    await page.screenshot({
      path: `test-results/visual/theme-${viewport.name}-dark.png`,
      fullPage: true,
      animations: "disabled",
    });
