  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path.endsWith("/territories")) {
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          items: [{ id: "territory-1", name: "Centro", code: "CEN" }],
    ...
    if (path.endsWith("/sellers")) {
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          items: [{ id: "seller-1", displayName: "Ana Vendedora" }],
          page: { page: 0, pageSize: 100, totalElements: 1, totalPages: 1 },
        }),
      });
      return;
...
for (const viewport of [
  ...
  test(`filtros de clientes alineados en ${viewport.name}`, async ({
    page,
  }) => {
    ...
    await page
      .getByRole("button", { name: "Gestión de clientes", exact: true })
      .click();
    ...
    if (viewport.name === "desktop") {
      ...
      const tops = await Promise.all(
        controls.map(async (control) =>
          Math.round((await control.boundingBox())?.y ?? -1),
        ),
      );
