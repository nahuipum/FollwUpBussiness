...
| `SEC-BE055-04` observabilidad de backlog incompleta | Medium | Gauges `outbox.backlog` y `outbox.oldest_pending_age_seconds`; reglas Prometheus para backlog, antigüedad, fallos y terminales en `infrastructure/monitoring/alerts/followupbussiness-outbox-alerts.yaml`. |
