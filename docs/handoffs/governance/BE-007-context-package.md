# Paquete de Contexto de Historia — BE-007 — v1
...
| Commit o diff candidato | Base Git `f320938d55f8ca9bf58d0df0bab259749ca5974e` en rama `feature/first`; worktree limpio antes de crear este artefacto. El paquete de gobernanza queda excluido del cambio funcional candidato. |
...
|---|---|---|---|
| Desarrollo | Este paquete v1; candidato base fijado | `docs/handoffs/development/BE-007-development-handoff.md` con `READY_FOR_HANDOFF` | Implementación y pruebas dirigidas; declarar lecturas excepcionales. |
| QA | Paquete vigente + handoff Dev + candidato actualizado | `docs/handoffs/qa/BE-007-qa-handoff.md` con `PASS` | Matriz criterio → prueba y revisión independiente. |
| Seguridad | Paquete vigente + handoff QA + candidato actualizado | `docs/handoffs/security/BE-007-security-handoff.md` con `PASS` | Revisión obligatoria de autorización, multiempresa y auditoría. |
| DoF | Paquete vigente + todos los handoffs + PR/CI | `docs/handoffs/dof/BE-007-dof-handoff.md` con `PASS` o `BLOCKED` | Mismo candidato, commit revisable, PR trazable y CI asociado. |
