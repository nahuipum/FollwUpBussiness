...
Wall time: 0.4 seconds
Output:
# Paquete de Contexto de Historia — BE-007 — v5
...
| Commit o diff candidato | Commit revisable `f8cf15e6768d8d3facb0ecf7e4303e24f4ccde6a` en `feature/first`, que contiene BE-007 y la corrección de `docker-compose.yml`; PR #8 y CI asociados. |
| Creado por | Agente Orquestador |
| Vigente hasta | Cambio de fuente o candidato |

## Cambio respecto de v4

- Único delta a validar: `docker-compose.yml` inyecta el secreto local con el nombre de entorno canónico que Spring enlaza (`FOLLOWUPBUSSINESS_SECURITY_LOCALSECRET`), conservando el nombre externo de `.env` sin persistir secretos.
- Evidencia técnica disponible: el stack arrancó; backend, PostGIS, Redis, RabbitMQ y Prometheus están activos; PostGIS/Redis/RabbitMQ saludables; backend validó Flyway V1–V7.
- No se releyeron fuentes primarias; no cambian reglas funcionales, contratos, ADR, migraciones ni código backend.
- Candidato QA/Seguridad/DoF: commit `f8cf15e6768d8d3facb0ecf7e4303e24f4ccde6a`; cualquier modificación posterior exige v6.
...
| QA | Este paquete v5 + cambio Compose + candidato fijado | `docs/handoffs/qa/BE-007-qa-handoff.md` con `PASS` | Matriz criterio → prueba y revisión independiente. |
| Seguridad | Este paquete v5 + handoff QA Compose + candidato fijado | `docs/handoffs/security/BE-007-security-handoff.md` con `PASS` | Revisión obligatoria de autorización, multiempresa y auditoría. |
| DoF | Este paquete v5 + todos los handoffs + PR/CI | `docs/handoffs/dof/BE-007-dof-handoff.md` con `PASS` o `BLOCKED` | Mismo candidato, commit revisable, PR trazable y CI asociado. |
