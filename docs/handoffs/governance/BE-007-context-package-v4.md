...
Output:
# Paquete de Contexto de Historia — BE-007 — v4
...
| Commit o diff candidato | Base/HEAD Git `f320938d55f8ca9bf58d0df0bab259749ca5974e` en rama `feature/first` + diff funcional Backend BE-007 no indexado con SHA-256 `261c12f5907fd534b6531095746d3108ec9c7f6caaefd688af9549d10b965c69`; `git diff --check` PASS. Excluye artefactos de gobernanza y handoffs. |
| Creado por | Agente Orquestador |
| Vigente hasta | Cambio de fuente o candidato |

## Cambio respecto de v3

- Motivo: se remedia el hallazgo obligatorio `SEC-BE007-001` de Seguridad: para actores de empresa el autenticador revalida en cada solicitud que la empresa durable existe y está `ACTIVE`; mantiene el caso de plataforma con tenant nulo explícito.
- Handoff de origen: `docs/handoffs/development/BE-007-security-remediation-handoff.md` (`READY_FOR_HANDOFF`), tras `docs/handoffs/security/BE-007-security-handoff.md` (`CHANGES_REQUIRED`).
- No cambió ninguna fuente funcional, contractual, ADR, OpenAPI ni migración; no se registraron lecturas primarias excepcionales en la remediación.
- Candidato QA/Seguridad/DoF: el diff funcional identificado arriba; cualquier modificación posterior exige v5.
...
| QA | Este paquete v4 + handoff de remediación de seguridad + candidato fijado | `docs/handoffs/qa/BE-007-qa-handoff.md` con `PASS` | Matriz criterio → prueba y revisión independiente. |
| Seguridad | Este paquete v4 + handoff QA de revalidación + candidato fijado | `docs/handoffs/security/BE-007-security-handoff.md` con `PASS` | Revisión obligatoria de autorización, multiempresa y auditoría. |
| DoF | Este paquete v4 + todos los handoffs + PR/CI | `docs/handoffs/dof/BE-007-dof-handoff.md` con `PASS` o `BLOCKED` | Mismo candidato, commit revisable, PR trazable y CI asociado. |
