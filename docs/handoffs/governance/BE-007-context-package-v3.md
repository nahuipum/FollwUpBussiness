...
Output:
# Paquete de Contexto de Historia — BE-007 — v3
...
| Commit o diff candidato | Base/HEAD Git `f320938d55f8ca9bf58d0df0bab259749ca5974e` en rama `feature/first` + diff funcional Backend BE-007 no indexado con SHA-256 `d5697f492c8ef7488cc4d6d4986b6632c5cd4dec2bee2777b82e23d3b5e62057`; `git diff --check` PASS. Excluye artefactos de gobernanza y handoffs. |
| Creado por | Agente Orquestador |
| Vigente hasta | Cambio de fuente o candidato |

## Cambio respecto de v2

- Motivo: se remediaron los hallazgos QA críticos de v2: ningún grant puede ampliar el alcance obligatorio de `SELLER`/`SUPERVISOR`, y la auditoría recibe el `correlationId` real de la operación.
- Handoff de origen: `docs/handoffs/development/BE-007-remediation-handoff.md` (`READY_FOR_HANDOFF`), tras `docs/handoffs/qa/BE-007-qa-handoff.md` (`CHANGES_REQUIRED`).
- No cambió ninguna fuente funcional, contractual, ADR, seguridad, OpenAPI ni migración; no se registraron lecturas primarias excepcionales en la remediación.
- Candidato QA/Seguridad/DoF: el diff funcional identificado arriba; cualquier modificación posterior exige v4.
...
| QA | Este paquete v3 + handoff de remediación + candidato fijado | `docs/handoffs/qa/BE-007-qa-handoff.md` con `PASS` | Matriz criterio → prueba y revisión independiente. |
| Seguridad | Este paquete v3 + handoff QA de revalidación + candidato fijado | `docs/handoffs/security/BE-007-security-handoff.md` con `PASS` | Revisión obligatoria de autorización, multiempresa y auditoría. |
| DoF | Este paquete v3 + todos los handoffs + PR/CI | `docs/handoffs/dof/BE-007-dof-handoff.md` con `PASS` o `BLOCKED` | Mismo candidato, commit revisable, PR trazable y CI asociado. |
