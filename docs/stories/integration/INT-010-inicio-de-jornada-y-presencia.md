...
- **Predecesoras obligatorias:** `BE-028` — Iniciar jornada; `BE-029` — Recibir ubicaciones; `BE-030` — Mantener última ubicación en Redis; `BE-031` — Publicar ubicación por WebSocket; `FE-020` — Mapa en tiempo real; `MOB-007` — Iniciar jornada; `MOB-008` — Capturar ubicación en segundo plano; `MOB-030` — Manejar batería y servicios desactivados
