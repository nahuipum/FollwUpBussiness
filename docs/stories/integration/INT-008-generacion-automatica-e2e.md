...
- **Predecesoras obligatorias:** `BE-022` — Generar ruta automática básica; `BE-023` — Reordenar puntos de ruta; `BE-027` — Sugerir clientes por frecuencia; `BE-061` — Consultar rutas y ruta del día; `FE-015` — Crear ruta manual; `FE-016` — Generar ruta automática
