...
- **Predecesoras obligatorias:** `BE-026` — Duplicar ruta; `BE-061` — Consultar rutas y ruta del día; `FE-003` — Gestión de sesión; `FE-034` — Manejo global de errores y permisos
- **Historias consecuentes que habilita:** `FE-015` — Crear ruta manual; `FE-016` — Generar ruta automática; `FE-018` — Reasignar ruta; `FE-019` — Comparar ruta planificada y ejecutada; `INT-007` — Creación manual E2E
