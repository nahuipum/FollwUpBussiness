...
public class CompanyUserService {
    ...
    public CompanyUserService(JdbcTemplate jdbc, Clock clock) {
        this(jdbc, clock, null, null, null, null, null);
    }
    ...
    public CompanyUserService(JdbcTemplate jdbc, Clock clock, PasswordRecoveryPort recovery, IdentityNotificationPort notifications,
        ...
        this.audit = audit;
        this.outbox = outbox;
        this.hmac = hmac == null ? null : hmac.clone();
    }
    ...
    public User get(UUID id, AuthenticatedActor actor, UUID correlationId) {
        return get(id, actor);
    }
    ...
    public User update(UUID id, Update command, AuthenticatedActor actor, UUID correlationId) {
        ...
        BaseRole role = command.role() == null ? old.role() : command.role();
        validRole(role);
        ...
        try {
            ...
            durableSuccess(actor, updated, old.status(), updated.updatedAt(), correlationId);
            return updated;
        } catch (DuplicateKeyException e) {
            throw new Conflict();
        }
    ...
    private UUID admin(AuthenticatedActor actor) {
        UUID tenant = tenant(actor);
        if (actor.role() != BaseRole.COMPANY_ADMIN) throw new Forbidden();
        return tenant;
    }

    private static void validRole(BaseRole role) {
        if (role != BaseRole.COMPANY_ADMIN && role != BaseRole.SUPERVISOR) throw new Invalid();
    }

    private static void validInvite(Invite command) {
        if (command == null) throw new Invalid();
        validName(command.name());
        validEmail(command.email());
        if (command.username() != null) validUsername(command.username());
    }

    private static void validUpdate(Update command) {
        if (command == null) throw new Invalid();
        if (command.name() != null) validName(command.name());
        if (command.email() != null) validEmail(command.email());
        if (command.username() != null) validUsername(command.username());
        if (command.role() != null) validRole(command.role());
    }

    private static String canonical(String value) {
        rejectCrLf(value);
        if ((value = value.strip().toLowerCase(Locale.ROOT)).isBlank()) throw new Invalid();
        return value;
    }

    private static String validName(String value) {
        rejectCrLf(value);
        if (value == null || (value = value.strip()).length() < 2 || value.length() > 160) throw new Invalid();
        return value;
    }

    private static String validUsername(String value) {
        rejectCrLf(value);
        if (value == null || (value = value.strip().toLowerCase(Locale.ROOT)).length() < 3 || value.length() > 100)
            throw new Invalid();
        return value;
    }

    private static String validEmail(String value) {
        String email = canonical(value);
        if (email.length() > 254 || !email.matches("^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$")) throw new Invalid();
        return email;
    }

    private static void rejectCrLf(String value) {
    ...
    private static User user(java.sql.ResultSet r) throws java.sql.SQLException {
        return new User(r.getObject(1, UUID.class), r.getString(2), r.getString(3), r.getString(4), BaseRole.valueOf(r.getString(5)), r.getString(6), r.getTimestamp(7).toInstant(), r.getTimestamp(8).toInstant(), r.getLong(9));
    }
    ...
    public record User(UUID id, @JsonProperty("displayName") String name, String username, String email, BaseRole role,
