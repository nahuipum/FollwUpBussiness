...
public class CreateCustomerService {
    ...
    public CreateCustomerService(CustomerStore store, TerritoryReferenceUseCase territories, RecordAuditEntryUseCase audit, Clock clock) {
        this.store = store;
    ...
    public Customer create(Command command, AuthenticatedActor actor) {
        ...
        if (command.territoryId() != null && !territories.activeTerritory(tenant, command.territoryId()))
            throw new InvalidTerritory();
        var now = clock.instant();
        Customer customer = new Customer(UUID.randomUUID(), tenant, clean(command.name()), optional(command.documentType()), optional(command.documentNumber()), optional(command.phone()), optional(command.email()), optional(command.segment()), clean(command.address()), command.location(), command.visitFrequencyDays(), command.territoryId(), "ACTIVE", now, now, 1);
        Customer saved = store.insert(customer);
        if (!audit.record(new RecordAuditEntryCommand(AuditAction.CRITICAL_MUTATION, AuditResourceType.CUSTOMER, saved.id(), AuditResult.SUCCESS, Map.of(), Map.of("status", "ACTIVE"))))
    ...
    private static String clean(String value) {
        if (value == null || value.trim().isEmpty()) throw new Invalid();
        return value.trim();
    }
    ...
    public record Command(String name, String documentType, String documentNumber, String phone, String email,
                          String segment, String address, GeoPoint location, Integer visitFrequencyDays,
                          UUID territoryId) {
    }
