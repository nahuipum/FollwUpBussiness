...
public final class SecurityContextCompanyDenialAuditTrustedContextProvider implements CompanyDenialAuditTrustedContextProvider {
    ...
    public SecurityContextCompanyDenialAuditTrustedContextProvider(Clock clock) {
        this.clock = clock;
    }

    @Override
    public CompanyDenialAuditTrustedContext current() {
        ...
        if (authentication == null || !authentication.isAuthenticated() || !(authentication.getPrincipal() instanceof AuthenticatedActor actor)
                || actor.tenantId() == null)
            throw new SecurityException("An authenticated tenant-bound actor is required for company denial audit recording");
