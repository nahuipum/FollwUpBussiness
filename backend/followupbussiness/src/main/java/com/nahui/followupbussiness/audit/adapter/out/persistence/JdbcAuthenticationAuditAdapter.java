...
public final class JdbcAuthenticationAuditAdapter implements RecordAuthenticationAuditUseCase {
    ...
    public JdbcAuthenticationAuditAdapter(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    @Override
    public void record(RecordAuthenticationAuditCommand c) {
        String status = switch (c.result()) {
            case REFRESHED, LOGGED_OUT -> "SUCCESS";
            case REUSED, REJECTED, RATE_LIMITED -> "DENIED";
            default -> "ERROR";
        };
