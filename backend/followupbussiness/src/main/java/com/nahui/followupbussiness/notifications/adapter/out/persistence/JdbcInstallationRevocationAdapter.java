...
public final class JdbcInstallationRevocationAdapter implements RevokeInstallationsForSession {
    ...
    public JdbcInstallationRevocationAdapter(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public void revoke(UUID family, UUID tenant) {
        jdbc.update("UPDATE notification_installation SET revoked_at=COALESCE(revoked_at,?) WHERE session_family_id=? AND tenant_id IS NOT DISTINCT FROM ?", Timestamp.from(Instant.now()), family, tenant);
    }
}
