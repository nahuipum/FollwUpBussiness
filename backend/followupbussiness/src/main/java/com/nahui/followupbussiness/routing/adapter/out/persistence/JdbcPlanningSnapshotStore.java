...
public final class JdbcPlanningSnapshotStore implements PlanningSnapshotStore {
 ...
 public void saveValid(PlanningSnapshot snapshot){try{jdbc.update("insert into route_planning_snapshot(id,tenant_id,route_id,base_route_version,status,valid_until,payload,created_at,updated_at) values(?,?,?,?, 'VALID', ?, ?::jsonb, now(), now())",snapshot.id(),snapshot.tenantId(),snapshot.routeId(),snapshot.baseRouteVersion(),Timestamp.from(snapshot.validUntil()),json.writeValueAsString(snapshot));}catch(Exception e){throw new IllegalStateException("planning snapshot persistence failed",e);}}
 public void saveIncomplete(UUID tenant,UUID route,long version,java.time.Instant validUntil){jdbc.update("insert into route_planning_snapshot(id,tenant_id,route_id,base_route_version,status,valid_until,payload,created_at,updated_at) values(?,?,?,?, 'INCOMPLETE', ?, '{}'::jsonb, now(), now())",UUID.randomUUID(),tenant,route,version,Timestamp.from(validUntil));}
 public void invalidateValidForTenant(UUID tenant){jdbc.update("update route_planning_snapshot set status='INVALIDATED',updated_at=now() where tenant_id=? and status='VALID'",tenant);}
