...
public final class JdbcRefreshSessionAdapter implements RefreshSessionPort {
    ...
    public JdbcRefreshSessionAdapter(JdbcTemplate jdbc) { this.jdbc = jdbc; }
    ...
    public Rotation rotate(Resolution f, byte[] presented, byte[] next, byte[] nextCsrf, Instant now) {
        ...
        if (updated == 1) jdbc.update("INSERT INTO identity_access_consumed_refresh_token(refresh_token_digest,family_id,consumed_at,channel,client_instance_digest) VALUES (?,?,?,?,?)", presented,f.familyId(),Timestamp.from(now),f.channel(),f.clientDigest());
        return new Rotation(updated == 1, now);
    }
    public void revoke(UUID id, Instant now) { jdbc.update("UPDATE identity_access_session_family SET revoked_at=COALESCE(revoked_at,?) WHERE id=?", Timestamp.from(now), id); }
}
