...
public final class RevokeInstallationsForSessionService implements RevokeInstallationsForSession {
    ...
    public RevokeInstallationsForSessionService(RevokeInstallationsForSession store) {
        this.store = store;
    }

    public void revoke(UUID family, UUID tenant) {
        if (family == null) throw new IllegalArgumentException("session family is required");
        store.revoke(family, tenant);
    }
}
