...
public final class JdbcCustomerStore implements CustomerStore {
    ...
    public List<DuplicateMatch> findDuplicateMatches(UUID tenantId, DuplicateCriteria criteria) {
        ...
        String sql = """
                select id,tenant_id,name,document_type,document_number,phone,email,address,ST_Y(location),ST_X(location),visit_frequency_days,territory_id,status,created_at,updated_at,version,
        ...
        return jdbc.query(sql, (rs, row) -> {
            Customer customer = new Customer(UUID.fromString(rs.getString(1)), UUID.fromString(rs.getString(2)), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), new com.nahui.followupbussiness.customers.domain.GeoPoint(rs.getDouble(9), rs.getDouble(10)), (Integer) rs.getObject(11), rs.getObject(12, UUID.class), rs.getString(13), rs.getTimestamp(14).toInstant(), rs.getTimestamp(15).toInstant(), rs.getLong(16));
            Set<MatchedField> fields = new LinkedHashSet<>();
            if (rs.getBoolean(17)) fields.add(MatchedField.DOCUMENT);
            if (rs.getBoolean(18)) fields.add(MatchedField.PHONE);
            if (rs.getBoolean(19)) fields.add(MatchedField.NAME);
            if (rs.getBoolean(20)) fields.add(MatchedField.ADDRESS);
            if (rs.getBoolean(21)) fields.add(MatchedField.LOCATION);
