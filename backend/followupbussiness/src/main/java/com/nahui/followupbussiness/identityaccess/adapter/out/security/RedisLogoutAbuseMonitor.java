...
public final class RedisLogoutAbuseMonitor implements LogoutAbuseMonitor {
    ...
    public RedisLogoutAbuseMonitor(StringRedisTemplate redis, byte[] secret) {
        this.redis = redis;
        this.secret = secret.clone();
    }

    public Decision recordGlobal(UUID account, UUID tenant) {
        try {
            String key = "auth:logout:dedupe:" + digest(account + ":" + tenant);
            Long n = redis.opsForValue().increment(key);
            if (n != null && n == 1) redis.expire(key, Duration.ofHours(1));
            long attempts = n == null ? 0 : n;
            return new Decision(attempts > 5, attempts);
