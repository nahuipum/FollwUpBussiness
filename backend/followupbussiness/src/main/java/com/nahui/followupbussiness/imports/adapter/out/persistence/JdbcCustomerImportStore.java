...
public final class JdbcCustomerImportStore implements CustomerImportStore {
    ...
    @Override public Optional<ClaimedImport> claim(UUID id, UUID tenant) {
    ...
    @Override public void complete(UUID id, int accepted, int rejected, boolean failed) {
    ...
    @Override public void recordRowErrors(UUID id, List<RowError> errors) {
        for (RowError error : errors) jdbc.update("INSERT INTO customer_import_row_error(id,import_id,row_number,error_code,created_at) VALUES (?,?,?,?,now()) ON CONFLICT(import_id,row_number,error_code) DO NOTHING", UUID.randomUUID(), id, error.rowNumber(), error.code());
    }
    @Override public Optional<CustomerImport> fail(UUID id, UUID tenant) {
    ...
    @Override public int purgeExpiredFiles() { return jdbc.update("UPDATE customer_import SET original_file=NULL WHERE original_file IS NOT NULL AND terminal_at < now()-interval '24 hours'"); }
    @Override public int purgeExpiredRowErrors() { return jdbc.update("DELETE FROM customer_import_row_error WHERE import_id IN (SELECT id FROM customer_import WHERE terminal_at < now()-interval '30 days')"); }
    private CustomerImport map(ResultSet r, int n) throws SQLException { return new CustomerImport(r.getObject("id",UUID.class),r.getObject("tenant_id",UUID.class),r.getObject("requested_by",UUID.class),r.getObject("correlation_id",UUID.class),r.getString("idempotency_key"),r.getString("file_name"),r.getString("content_type"),r.getString("template_version"),r.getBoolean("partial_acceptance"),r.getString("file_sha256"),CustomerImport.Status.valueOf(r.getString("status")),r.getInt("accepted_rows"),r.getInt("rejected_rows"),r.getTimestamp("created_at").toInstant(),r.getTimestamp("updated_at").toInstant(),r.getTimestamp("terminal_at") == null ? null : r.getTimestamp("terminal_at").toInstant(),r.getTimestamp("error_file_expires_at") == null ? null : r.getTimestamp("error_file_expires_at").toInstant()); }
}
