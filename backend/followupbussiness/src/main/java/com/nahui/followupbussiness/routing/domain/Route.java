...
public record Route(UUID id, UUID tenantId, String name, LocalDate date, UUID sellerId, GeoPoint startLocation,
    ...
    public Route {
        ...
        for (int index = 0; index < points.size(); index++)
            if (points.get(index).sequence() != index + 1) throw new IllegalArgumentException("invalid point sequence");
    }

    public Route(UUID id, UUID tenantId, String name, LocalDate date, UUID sellerId, GeoPoint startLocation,
