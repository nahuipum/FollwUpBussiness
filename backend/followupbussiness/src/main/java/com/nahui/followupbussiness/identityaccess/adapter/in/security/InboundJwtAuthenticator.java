...
public final class InboundJwtAuthenticator {
    ...
    public InboundJwtAuthenticator(PublicKey publicKey, String issuer, String audience, JdbcTemplate jdbcTemplate, Clock clock) {
        this.publicKey = publicKey; this.issuer = issuer; this.audience = audience; this.jdbcTemplate = jdbcTemplate; this.clock = clock;
    }
    ...
    public Authentication authenticate(String bearerToken) {
        try {
            ...
            JsonNode header = parse(parts[0]); JsonNode claims = parse(parts[1]);
            ...
            if (!issuer.equals(claims.path("iss").asText()) || !hasAudience(claims) || claims.hasNonNull("tid") || claims.path("exp").asLong(0) <= clock.instant().getEpochSecond()) throw new JwtValidationException();
    ...
    private JsonNode parse(String encoded) { try { return json.readTree(Base64.getUrlDecoder().decode(encoded)); } catch (Exception exception) { throw new JwtValidationException(); } }
    private boolean verify(String[] parts) throws GeneralSecurityException { Signature signature = Signature.getInstance("SHA256withRSA"); signature.initVerify(publicKey); signature.update((parts[0] + "." + parts[1]).getBytes(StandardCharsets.US_ASCII)); return signature.verify(Base64.getUrlDecoder().decode(parts[2])); }
    private String singleRole(JsonNode claims) { JsonNode roles = claims.path("roles"); if (!roles.isArray() || roles.size() != 1 || !"PLATFORM_SUPERADMIN".equals(roles.get(0).asText())) throw new JwtValidationException(); return roles.get(0).asText(); }
    private boolean hasAudience(JsonNode claims) { JsonNode audiences = claims.path("aud"); if (audiences.isTextual()) return audience.equals(audiences.asText()); if (audiences.isArray()) for (JsonNode value : audiences) if (audience.equals(value.asText())) return true; return false; }

    public static final class JwtValidationException extends org.springframework.security.core.AuthenticationException {
        public JwtValidationException() { super("Invalid access token"); }
    }
}
