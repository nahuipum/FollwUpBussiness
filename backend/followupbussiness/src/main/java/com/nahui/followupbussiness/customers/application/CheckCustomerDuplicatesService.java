...
public final class CheckCustomerDuplicatesService {
    ...
    public CheckCustomerDuplicatesService(CustomerStore store) { this.store = store; }
    ...
    private static String required(String value) { String result = optional(value); if (result == null) throw new Invalid(); return result; }
    private static String optional(String value) { if (value == null || value.trim().isEmpty()) return null; return value.trim().toLowerCase(Locale.ROOT); }
    private static String identity(String value) { String result = optional(value); return result == null ? null : result.replace(" ", "").replace("-", ""); }
    public record Command(String name, String documentNumber, String phone, String address, GeoPoint location, UUID excludeCustomerId) {}
    public record Candidate(com.nahui.followupbussiness.customers.domain.Customer customer, java.util.Set<CustomerStore.MatchedField> matchedFields, double score) {}
    public record Result(List<Candidate> candidates) { public boolean hasPossibleDuplicates() { return !candidates.isEmpty(); } }
