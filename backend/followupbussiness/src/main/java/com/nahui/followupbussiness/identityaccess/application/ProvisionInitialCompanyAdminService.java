...
public final class ProvisionInitialCompanyAdminService implements ProvisionInitialCompanyAdminUseCase {
    ...
    private final InitialCompanyAdminStore accounts;
    ...
    @Override
    public ProvisionInitialCompanyAdminResult execute(ProvisionInitialCompanyAdminCommand command, AuthenticatedActor actor) {
        if (actor == null || actor.role() != BaseRole.PLATFORM_SUPERADMIN || actor.tenantId() != null)
            throw new Forbidden();
        if (command == null || command.companyId() == null || !companies.isActive(command.companyId()))
            throw new CompanyUnavailable();
        String email = canonical(command.email());
        String username = canonical(command.username() == null || command.username().isBlank() ? email : command.username());
        String displayName = command.displayName() == null ? null : command.displayName().strip();
        if (displayName == null || displayName.length() < 2 || displayName.length() > 160 || email == null || username == null)
            throw new Invalid();
        UUID id = UUID.randomUUID();
        char[] unusable = secret().toCharArray();
        try {
            if (!accounts.create(id, command.companyId(), username, passwords.hash(unusable), displayName, email))
                throw new Conflict();
        } finally {
            java.util.Arrays.fill(unusable, '\0');
        }
        String token = secret();
        var expires = clock.instant().plus(Duration.ofHours(24));
    ...
    private static String canonical(String value) {
        ...
        String x = value.strip().toLowerCase(Locale.ROOT);
    ...
    private byte[] digest(String value) {
        try {
            ...
            throw new IllegalStateException("Unable to digest activation token", e);
        }
    }
    ...
    public static final class CompanyUnavailable extends RuntimeException {
    }

    public static final class Conflict extends RuntimeException {
    }

    public static final class DeliveryUnavailable extends RuntimeException {
    }
