...
class CustomerControllerTest {
    private CreateCustomerService service; private MockMvc mvc;
    @BeforeEach void setUp() { service = mock(CreateCustomerService.class); mvc = MockMvcBuilders.standaloneSetup(new CustomerController(service)).setControllerAdvice(new CustomerValidationErrorHandler()).setCustomArgumentResolvers(new AuthenticationPrincipalArgumentResolver()).build(); }
    ...
    @Test void createsOnlyForSessionAdminAndRejectsUnknownProperties() throws Exception {
        ...
        when(service.create(any(), any())).thenReturn(new Customer(id, tenant, "Customer", null, null, null, null, "Address", new GeoPoint(-12.1, -77.1), null, null, Instant.EPOCH, Instant.EPOCH, 1));
