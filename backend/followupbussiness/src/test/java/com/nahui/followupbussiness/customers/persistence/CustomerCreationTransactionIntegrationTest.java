...
class CustomerCreationTransactionIntegrationTest {
    ...
    @Test
    void storesWgs84Point() {
        try (var context = context()) {
            var service = context.getBean(CreateCustomerService.class);
    ...
    @Test
    void rollsBackCustomerAndAuditWhenCommitFailsAfterAuditWasWritten() {
    ...
    private AnnotationConfigApplicationContext context() {
        ...
        c.registerBean(JdbcTemplate.class, () -> jdbc);
        c.registerBean(TerritoryReferenceUseCase.class, () -> (tenantId, territoryId) -> true);
        c.registerBean(PlatformTransactionManager.class, () -> new DataSourceTransactionManager(dataSource));
        c.registerBean(AuditTrustedContextProvider.class, () -> () -> new AuditTrustedContext(tenant, UUID.randomUUID(), UUID.randomUUID(), com.nahui.followupbussiness.audit.domain.AuditScope.AUTHORIZED_RESOURCE));
        c.register(Config.class);
        c.refresh();
        return c;
    }

    private CreateCustomerService.Command command() {
        return new CreateCustomerService.Command("Customer", null, null, null, null, "STANDARD", "Address", new GeoPoint(-12.1, -77.1), null, null);
    }

    private AuthenticatedActor actor() {
        return new AuthenticatedActor(UUID.randomUUID(), tenant, BaseRole.COMPANY_ADMIN);
    }
    ...
    static class Config {
        ...
        RecordAuditEntryUseCase transactionalAuditEntryUseCase(JdbcTemplate jdbc, AuditTrustedContextProvider contextProvider) {
