...
class SellerStatusServiceTest {
    ...
    void inactivationRevokesThroughIdentityAndPreservesSellerRelations() {
        Store store = new Store(seller(TerritoryStatus.ACTIVE, 4));
        ...
        Seller changed = service(store, users, command -> { audit[0] = command; return true; })
                .status(store.value.id(), TerritoryStatus.INACTIVE, "offboarding approved", admin, UUID.randomUUID());

        assertEquals(TerritoryStatus.INACTIVE, changed.status());
    ...
    void rejectsUnauthorizedCrossTenantRepeatedAndInvalidWithoutIdentityWritesOrAudit() {
        Store store = new Store(seller(TerritoryStatus.ACTIVE, 1));
        Users users = new Users();
        int[] audits = new int[1];
        SellerService service = service(store, users, command -> { audits[0]++; return true; });

        assertThrows(SellerService.Forbidden.class, () -> service.status(store.value.id(), TerritoryStatus.INACTIVE, "valid reason", new AuthenticatedActor(UUID.randomUUID(), tenant, BaseRole.SUPERVISOR), UUID.randomUUID()));
        assertThrows(SellerService.NotFound.class, () -> service.status(store.value.id(), TerritoryStatus.INACTIVE, "valid reason", new AuthenticatedActor(UUID.randomUUID(), UUID.randomUUID(), BaseRole.COMPANY_ADMIN), UUID.randomUUID()));
        assertThrows(SellerService.Invalid.class, () -> service.status(store.value.id(), TerritoryStatus.INACTIVE, "bad", admin, UUID.randomUUID()));
        store.value = seller(TerritoryStatus.INACTIVE, 1);
        assertThrows(SellerService.Conflict.class, () -> service.status(store.value.id(), TerritoryStatus.INACTIVE, "valid reason", admin, UUID.randomUUID()));
    ...
    void concurrentStatusWriteConflictDoesNotAudit() {
        ...
        assertThrows(SellerService.Conflict.class, () -> service(store, users, command -> { audits[0]++; return true; })
                .status(store.value.id(), TerritoryStatus.INACTIVE, "valid reason", admin, UUID.randomUUID()));
    ...
    void inactiveSellerRejectsFutureAssignmentBeforeAnyWrite() {
        Seller inactive = seller(TerritoryStatus.INACTIVE, 1);
    ...
    private Seller seller(TerritoryStatus status, long version) {
    ...
    private static final class Store implements SellerStore {
        ...
        @Override public Optional<Seller> updateStatus(Seller seller, TerritoryStatus expectedStatus) {
