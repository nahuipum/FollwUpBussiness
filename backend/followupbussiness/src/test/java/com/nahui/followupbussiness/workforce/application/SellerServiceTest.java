...
class SellerServiceTest {
    ...
    @Test
    void onlyAdminCanCreateAndInactiveRelationCreatesNothing() {
        Store store = new Store();
        SellerService service = service(store);
    ...
    @Test
    void validAdminPersistsSeparateSellerAfterInvitedUser() {
        Store store = new Store();
        SellerService result = service(store);
        ...
        assertNotNull(seller.userId());
        assertSame(seller, store.saved);
        assertEquals(List.of(territory), seller.territoryIds());
    }

    @Test
    void auditFailureIsPropagated() {
        Store store = new Store();
        CompanyUserService users = new CompanyUserService(null, Clock.systemUTC()) {
            @Override
            public User inviteSeller(Invite i, AuthenticatedActor a, UUID c) {
                return new User(UUID.randomUUID(), i.name(), i.username(), i.email(), BaseRole.SELLER, "INVITED", Instant.now(), Instant.now(), 1);
            }
        };
    ...
    private SellerService service(Store store) {
        ...
        RecordAuditEntryUseCase audit = (RecordAuditEntryCommand x) -> true;
        return new SellerService(store, users, audit, Clock.systemUTC());
    }

    private AuthenticatedActor admin() {
        return new AuthenticatedActor(UUID.randomUUID(), tenant, BaseRole.COMPANY_ADMIN, null);
    }
    ...
    private final class Store implements SellerStore {
        ...
        public boolean activeSupervisor(UUID t, UUID id) {
            return active && tenant.equals(t) && supervisor.equals(id);
        }

        public boolean activeTerritory(UUID t, UUID id) {
            return active && tenant.equals(t) && territory.equals(id);
        }
        ...
        public Optional<Seller> find(UUID t, UUID id) {
