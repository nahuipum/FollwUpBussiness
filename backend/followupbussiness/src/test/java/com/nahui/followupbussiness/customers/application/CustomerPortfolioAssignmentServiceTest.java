...
class CustomerPortfolioAssignmentServiceTest {
  ...
  @Test void concurrentBatchesWithSameKeyReplayTheCompletedResultWithoutDuplicateEffects() throws Exception {
    ...
    try (ExecutorService executor=Executors.newFixedThreadPool(2)) {
      ...
      assertFalse(firstResult.replayed()); assertTrue(secondResult.replayed()); assertEquals(firstResult.items(),secondResult.items());
    }
  ...
  @Test void batchUsesPayloadFingerprintAndExactReplayWithoutEffects() {
    ...
    assertFalse(first.replayed()); assertTrue(replay.replayed()); assertEquals(first.items(),replay.items()); assertEquals(1,store.replacements); assertEquals(1,audits[0]);
  ...
  private CustomerPortfolioAssignmentService service(Store store, RecordAuditEntryUseCase audit) {
    CustomerStore customers=new CustomerStore() {
      ...
      @Override public boolean update(Customer value,long version) { return false; }
  ...
  private final class Store implements CustomerPortfolioStore {
    Set<UUID> current=new HashSet<>(); Map<UUID,IdempotencyRecord> records=new HashMap<>(); Set<UUID> pending=new HashSet<>(); int replacements,reservations,completions; boolean blockReplacement;
