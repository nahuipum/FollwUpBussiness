...
class ChangeCompanyStatusServiceTest {
    ...
    @Test
    void transitionsAuditOnlyReasonPresenceForAnyCallerSuppliedText() {
        for (String reason : java.util.List.of("Operational review", "admin@example.test", "token=opaque-value", "Bearer token-value", "api_key=demo-secret", "arbitrary free text")) {
            ...
            var result = service(candidateStore, candidateAudit, command -> {
            }).execute(companyId,
    ...
    @Test
    void sameStatusIsA200ResultWithoutWriteOrChangeAudit() {
        Store store = new Store(company(CompanyStatus.ACTIVE));
        CapturingAudit audit = new CapturingAudit();
        var result = service(store, audit, command -> {
        }).execute(companyId,
    ...
    @Test
    void reactivatesASuspendedCompanyWithTheInverseAuditedTransition() {
    ...
    @Test
    void tenantBoundActorIsDeniedAndAuditedWithoutTouchingCompany() {
        Store store = new Store(company(CompanyStatus.ACTIVE));
    ...
    private static final class Store implements CompanyStatusStore {
        ...
        private Store(Company company) {
            this.company = company;
        }

        @Override
        public Optional<Transition> changeStatus(UUID id, CompanyStatus desired, Instant changedAt) {
            calls++;
            ...
            writes++;
            return Optional.of(new Transition(company, after, true));
        }
    }
