...
class InitialCompanyAdminControllerTest {
    @Test
    void rejectsRoleTenantAndCompanyInjectionBeforeUseCase() throws Exception {
        ...
        for (String field : new String[]{"role", "tenantId", "companyId"})
            mvc.perform(post("/platform/companies/{id}/initial-admin", UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON)
