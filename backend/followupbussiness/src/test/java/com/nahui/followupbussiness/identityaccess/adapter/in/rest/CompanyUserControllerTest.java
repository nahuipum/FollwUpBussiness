...
class CompanyUserControllerTest {
    ...
    @Test void idRoutesBindTheOpenApiUserIdVariableAndReachTheUseCase() throws Exception {
        UUID id = UUID.randomUUID(); var user = user(id); when(service.get(eq(id), eq(actor), any())).thenReturn(user); when(service.update(eq(id), any(), eq(actor), any())).thenReturn(user); when(service.status(eq(id),eq("LOCKED"),eq(actor),any())).thenReturn(user);
    ...
    @Test void listReturnsContractPageAndBindsOnlyDeclaredFilters() throws Exception { when(service.list(1,2,"name",BaseRole.SUPERVISOR,"ACTIVE",actor)).thenReturn(new CompanyUserService.UserPage(List.of(user(UUID.randomUUID())),new CompanyUserService.PageInfo(1,2,3,2)));
        mvc.perform(get("/company/users?page=1&pageSize=2&search=name&role=SUPERVISOR&status=ACTIVE").principal(() -> "x")).andExpect(status().isOk()).andExpect(jsonPath("$.items[0].id").exists()).andExpect(jsonPath("$.page.page").value(1)).andExpect(jsonPath("$.page.totalElements").value(3)); verify(service).list(1,2,"name",BaseRole.SUPERVISOR,"ACTIVE",actor); }
