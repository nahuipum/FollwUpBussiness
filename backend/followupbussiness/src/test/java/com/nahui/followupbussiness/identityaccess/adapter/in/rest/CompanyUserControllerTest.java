...
class CompanyUserControllerTest {
    ...
    @BeforeEach
    void setUp() {
        service = mock(CompanyUserService.class);
        actor = new AuthenticatedActor(UUID.randomUUID(), UUID.randomUUID(), BaseRole.COMPANY_ADMIN);
        SecurityContextHolder.getContext().setAuthentication(UsernamePasswordAuthenticationToken.authenticated(actor, "test", List.of()));
    ...
    @Test
    void idRoutesBindTheOpenApiUserIdVariableAndReachTheUseCase() throws Exception {
        UUID id = UUID.randomUUID();
        var user = user(id);
        when(service.get(eq(id), eq(actor), any())).thenReturn(user);
        when(service.update(eq(id), any(), eq(actor), any())).thenReturn(user);
        when(service.status(eq(id), eq("LOCKED"), eq(actor), any())).thenReturn(user);
        ...
        verify(service).get(eq(id), eq(actor), any());
        verify(service).update(eq(id), any(), eq(actor), any());
        verify(service).status(eq(id), eq("LOCKED"), eq(actor), any());
    }

    @Test
    void malformedUserIdIsRejectedBeforeTheUseCase() throws Exception {
        mvc.perform(get("/company/users/not-a-uuid").principal(() -> "x")).andExpect(status().isBadRequest());
        verifyNoInteractions(service);
    }

    @Test
    void pendingInvitationUsesDedicatedRouteWithQuotedVersionAndReturnsAccepted() throws Exception {
        UUID id = UUID.randomUUID();
        var invited = new CompanyUserService.User(id, "Name", "name", "a@example.test", BaseRole.SUPERVISOR, "INVITED", Instant.EPOCH, Instant.EPOCH, 2);
    ...
    @Test
    void invitationRejectsUnquotedVersionBeforeTheUseCase() throws Exception {
        mvc.perform(post("/company/users/{userId}/invitation", UUID.randomUUID()).principal(() -> "x").header("If-Match", "2").contentType(MediaType.APPLICATION_JSON).content("{\"displayName\":\"Name\",\"email\":\"a@example.test\",\"role\":\"SUPERVISOR\"}")).andExpect(status().isBadRequest());
        verifyNoInteractions(service);
    }
    ...
    @Test
    void listReturnsContractPageAndBindsOnlyDeclaredFilters() throws Exception {
        when(service.list(1, 2, "name", BaseRole.SUPERVISOR, "ACTIVE", actor)).thenReturn(new CompanyUserService.UserPage(List.of(user(UUID.randomUUID())), new CompanyUserService.PageInfo(1, 2, 3, 2)));
        ...
        verify(service).list(1, 2, "name", BaseRole.SUPERVISOR, "ACTIVE", actor);
    }

    private static CompanyUserService.User user(UUID id) {
        return new CompanyUserService.User(id, "Name", "name", "a@example.test", BaseRole.SUPERVISOR, "ACTIVE", Instant.EPOCH, Instant.EPOCH, 1);
    }
}
